...
public class JdbcPluginRepository implements PluginRepository {
    ...
    public void addPlugin(PluginConfiguration configuration) {
        ...
        if (jdbcTemplate.queryForInt(PLUGIN_EXISTS, configuration.getPluginURI()) > 0) {           
            ...
            jdbcTemplate.update(PLUGIN_ADD, configuration.getPluginURI(), configuration.getPluginType(), configuration.getTitle(), configuration.getDescription(), configuration.getPluginClass());
        }
    ...
    public List<PluginConfiguration> listAvailablePlugins(String userURI, PluginType[] pts) {
        ...
        while (set.next()) {
            ...
            String pluginURI = set.getString("p.PluginURI");
            String pluginType = set.getString("p.PluginType");
            String className = set.getString("p.Classname");
            String title = set.getString("p.Title");
            String desc = set.getString("p.Description");
            String paramName = set.getString("pp.Name");
            String paramType = set.getString("pp.Type");
            String paramValue = set.getString("pip.Value");
            int pluginInstanceId = set.getInt("pi.PluginInstanceID");
            
            if (!output.containsKey(pluginURI)) {
               
                PluginConfiguration pc = new PluginConfiguration();
                pc.setPluginURI(pluginURI);
                pc.setPluginType(PluginType.valueOf(pluginType));
